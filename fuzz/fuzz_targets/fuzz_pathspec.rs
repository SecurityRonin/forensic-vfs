#![no_main]
//! The Locator URI parser is the leaf's attacker-controllable parse surface: a
...
fuzz_target!(|data: &[u8]| {
    ...
    if let Ok(spec) = Locator::from_uri(&s) {
        // Round-trip invariant: to_uri(from_uri(x)) re-parses to the same spec.
        let reparsed = Locator::from_uri(&spec.to_uri()).expect("own output must re-parse");
