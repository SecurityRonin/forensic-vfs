...
fn vfs_open_detects_and_mounts_apfs() {
    let ev = Vfs::new().open(Path::new(FIXTURE)).expect("open apfs fixture");
    let fs = ev.fs.expect("APFS should resolve to a filesystem");

    // The detected-stack locator names the APFS layer.
    assert!(ev.root.to_uri().contains("fs:apfs"), "locator: {}", ev.root.to_uri());
    ...
    let names: Vec<String> = walk(fs.as_ref())
        ...
        .collect();
    assert!(names.iter().any(|n| n == "plain.txt"), "walk should surface plain.txt: {names:?}");
}
