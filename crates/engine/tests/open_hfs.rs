...
fn vfs_open_detects_and_mounts_hfsplus() {
    let ev = Vfs::new().open(Path::new(FIXTURE)).expect("open hfs+ fixture");
    let fs = ev.fs.expect("HFS+ should resolve to a filesystem");

    assert!(ev.root.to_uri().contains("fs:hfsplus"), "locator: {}", ev.root.to_uri());
    ...
    let names: Vec<String> = walk(fs.as_ref())
        ...
        .collect();
    assert!(names.iter().any(|n| n.eq_ignore_ascii_case("HELLO.TXT")), "walk should surface HELLO.TXT: {names:?}");
}
