...
- `VolumeSystem::open_volume(index) -> DynSource` — a partition scheme yields a windowed
  sub-source.
- `EncryptionLayer::open(creds) -> DynSource` — an encryption layer yields a decrypted
  sub-source.
...
- Arbitrary stacks compose without a combinatorial matrix of adapters: N containers × M
  volume schemes × K encryption layers × J filesystems is N+M+K+J impls, not N·M·K·J.
- A layer is testable in isolation against any `ImageSource`, including an in-memory one.
- The two vertical layers (`VolumeSystem`, `EncryptionLayer`) currently have zero leaf impls;
